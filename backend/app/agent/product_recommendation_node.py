...
from app.recommendation.recommendation_tool import run_product_recommendation
...
def product_recommendation_node(state: AgentState) -> AgentState:
    logger.info("===== PRODUCT RECOMMENDATION NODE =====")

    df = pd.read_csv(settings.DATASET_PATH)
    intent = state["intent_data"]

    requested_features = (
        intent.get("features_requested") or
        [
            "age",
            "monthly_income",
            "avg_monthly_balance",
    ...
    valid_features = [f for f in requested_features if f in df.columns]
    ...
    for segment in results["segment_recommendations"]:
        cluster = next(
            c for c in results["cluster_centers"]
        ...
        summary = {
            ...
            "average_profile":{
                "age": round(cluster.get("age", 35), 1),
                "income": round(cluster.get("monthly_income", 50000), 2),
                "balance": round(cluster.get("avg_monthly_balance", 15000), 2),
                "credit_score": round(cluster.get("credit_score", 650), 1),
                "transaction_frequency": round(cluster.get("transaction_frequency", 10), 1)
            },
            "recommended_products":[
                ...
                for p in segment["recommended_products"]
            ]
        }
...
3. Explain WHY every recommended product matches that customer segment.
...
Use Markdown formatting. Do not use generic placeholders like [Insert Bank Name].
"""
    ...
    state["final_output"] = {
        ...
        "agent_reasoning":{
            "detected_intent": "recommend_products",
            "features_used": valid_features,
            "evaluation_metrics": results["silhouette_score"]
        },
    ...
    state["response_message"] = (
        ...
        "recommended banking products, AI business insights, and campaign strategies."
    )
